...
b = [*map(str.strip, sys.stdin)]; m = [['@']*(2*len(b[0])+1)]
for l in b: m.append([*('@'+'@'.join(l)+'@')]), m.append(['@']*(2*len(b[0])+1))
R, C = len(m), len(m[0])
d = [-2]*R*C; k = ((0, -1), (0, 1), (-1, 0), (1, 0)); s = [(0, 0)]
...
print('Part 1:', max(d)//2)
print('Part 2:', sum(d[C*r+c]<-1 for r in range(1, R, 2) for c in range(1, C, 2)))
