...
# Day 13: 257
import re;L=len(R:=[*map(int,re.findall('\d+',open(0).read()))]);[print(f'Part {p+1}:',sum([a:=R[i:i+6],b:=a[0],c:=a[4]+(u:=p*10**13),(x:=c*a[1]-b*a[5]-b*u)%(y:=a[2]*a[1]-b*a[3])==(d:=c-x//y*a[2])%b==0and d*3//b+x//y][3]for i in range(0,L,6)))for p in(0,1)]